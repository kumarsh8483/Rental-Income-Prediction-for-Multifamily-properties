...
#Support Vector Regression - Polynomial kernel for prediction of rental incomes
...
#polynomial
parameters = [{'kernel': ['poly'],'degree':[2,3,4,5],'C': [1,5,10,25,50,75,100],
               'epsilon':[0.1,0.2,0.3,0.5]}]
svr_ploy = SVR()
svr_gridsearch = GridSearchCV(svr_ploy, parameters,cv=5)
svr_gridsearch.fit(ind_scaled,dep_scaled)
svr_gridsearch.best_params_
svr_gridsearch.best_estimator_

sv2=SVR(C=1, cache_size=200, coef0=0.0, degree=2, epsilon=0.3, gamma='auto',
  kernel='poly', max_iter=-1, shrinking=True, tol=0.001, verbose=False)
sv2.fit(ind_scaled, dep_scaled)

#Cross-validated Train errors
temp = sv2.predict(ind_scaled)
...
temp1 = sv2.predict(ind_test_scaled)
